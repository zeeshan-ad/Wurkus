<?php
     require_once 'logindb.php';
        ...
        if (isset($_GET['log_out']) && isset($_COOKIE['SNID'])){
            ...
            if(($_SERVER["REQUEST_METHOD"] == "POST") && isset($_POST['brand_name']) && isset($_POST['category']) && isset($_POST['location']) && isset($_POST['description']) && ($_FILES['uploadedimage']['tmp_name'])==""){
                    ...
                    if($output['status']=="OK"){
                        ...
                        $query ="INSERT INTO sponsor_page (brand_name, location, lat, lon, category, website, description, image_path, unique_key, user_id) VALUES ('$brand_name','$location','$latitude','$longitude','$category','$website','$description','$image_path', '$unique_key', '$uid')";
                        $result_sponsor = mysqli_query($dbc,$query);
                    }
                    else{
                        $query ="INSERT INTO sponsor_page (brand_name, location, category, website, description, image_path, unique_key, user_id) VALUES ('$brand_name','$location','$category','$website','$description','$image_path', '$unique_key', '$uid')";
                        $result_sponsor = mysqli_query($dbc,$query);
                    }
...
<html lang="en">
    <head>
        ...
        <title>Create Sponsor Page | Wurkus</title>
...
//              This is the form that our event handler fills
                echo '<form method="post" class="coords" action="create-sponsor-page.php">
            ...
            <form method="post" enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
                <table>
                    <tr>
                        <td>
                <p>Page name</p>
                <input maxlength="22" type="text" name="brand_name" id="brand_name" required>
                        </td>
                        <td id="left-space">
                            <p>Location</p>
                ...
                <p>Page profile picture</p>
                 <input name="uploadedimage" id="uploadedimage" type="file" accept="image/*">
                <p>Description</p>
                <textarea maxlength="5000" id="description" name="description" placeholder="About page..." rows="5" cols="55" required></textarea>
