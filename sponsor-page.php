...
<?php
   require_once 'logindb.php';
        ...
        $first_name= ucfirst($firstname_row['first_name']);
        ...
        $(window).scroll(function(){
            ...
            $(document).ready(function(){
                ...
                var action = 'inactive';
                    ...
                    function load_posts_sponsors(limit,start,page_key){
                        $.ajax({
                            url : "fetch-sponsor-posts.php",
                            method : "POST",
                    ...
                    if(action == 'inactive'){
                        action = 'active';
                        load_posts_sponsors(limit,start,page_key);
                    }
                    $(window).scroll(function(){
    ...
    <body>
        <div id="wrapper">
          ...
          echo '<div id="nav-stick">';
            ...
            $profile_details= mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM sponsor_page WHERE unique_key='$key'"));
            if($uid != $profile_details['user_id']){
                echo '<div id="space-2"></div>';
            }
            ?>
            <!--findig length of location and fixing accordingly-->
            <?php 
            ...
            <div class="profile-detail" >
                ...
                <p><?php echo $profile_details['brand_name'];?> <span id="location-profile" title="<?php echo $long_details; ?>">(<?php  
                    echo $details;?>)
                    </span></p>
                </span>
               ...
               <?php if($uid == $profile_details['user_id']){
                echo '<span id="followed-by-sp"><button id="open_popup"><span id="num">';
                echo $follower;
                echo '</span>';  
                if($num_rows<=1){
                    echo ' follower';}
                ...
                <span id="about-profile"><p><span class="make_bold">About:</span> <?php
                    ...
                    echo nl2br($profile_details['description']);
                        ...
                        echo '<div id="focus_post">';
                            echo '<div id="descrip_post" class="shadow">';
                                     ...
                                     echo '<span id="public_post_name_rm"><a href="sponsor-page.php?key='.$key.'">'.$profile_details['brand_name'].'</a></span>';
                                    ...
                                    if($rows==0){
                                    echo '<a href="sponsor-page.php?key='.$key.'&val=0"><div id="post_follow_button">'; echo '<p>Follow</p>'; echo '</div></a>';
                                    } else{
                                       echo '<a href="sponsor-page.php?key='.$key.'&val=1"><div id="post_follow_button">'; echo '<p>Unfollow</p>'; echo '</a></div>'; 
                                    }
                                    ...
                                    echo '<span>'; echo '<a href="sponsor-page.php?key='.$key.'">Know more</a>'; echo '</span>';
                                    echo '</div>';
                                    echo '</div>';
                                        }
                                         else{       
                                    echo '<div id="links_public_post">';
                                    echo '<span>'; echo '<a href="sponsor-page.php?key='.$key.'">Know more</a>'; echo '</span>';
                                    echo '<span id="edit-profile-post"><a href="edit-sponsor-page.php?key='.$key.'" title="Edit my page">Edit</a></span>';         
                                    echo '</div>';
                                    echo '</div>';     
                                                }
                    ...
                    ?></p></span>
                            ...
                            if($uid != $profile_details['user_id']){
                              ...
                              //query to check if user is following this page
                                $user_id_contact=$profile_details['user_id'];
                                    if($rows==0){
                                    echo '<a href="sponsor-page.php?key='.$key.'&val=0"><div id="post_follow_button">'; echo '<p>Follow</p>'; echo '</div></a>';
                                    } else{
                                       echo '<a href="sponsor-page.php?key='.$key.'&val=1"><div id="post_follow_button">'; echo '<p>Unfollow</p>'; echo '</div></a>'; 
                                    }
                                echo '</div>';
                              ...
                              echo '<span id="contact">'; echo '<a href="message.php?id='.$user_id_contact.'">Contact</a>'; echo '</span>'; echo '</div>';
                                ...
                                echo '<div id="edit-profile"><a href="edit-sponsor-page.php?key='.$key.'" title="Edit my page">Edit</a></div>';
                            }
                ...
                if(isset($_GET['val'])){
                    if($_GET['val']==0){
                       ...
                       header('location:sponsor-page.php?key='.$key);
                    }else{
          ...
          <div class="modal-content">
            ...
            <p id="followPop_head">People who have followed this page</p>
              ...
              ?>
                ...
                if($uid == $profile_details['user_id']){
                echo "<div id='make-post' class='shadow'>
                <form method='post' class='form_submit' action='sponsor-page.php?key=$key#make-post'>
                ...
                if(isset($_POST['profile-post'])){
                    $post =  addslashes($_POST['profile-post']);
                    $res=mysqli_query($dbc,"INSERT INTO sponsor_make_post (post, time, date, user_id) VALUES ('$post', CURTIME(), CURDATE(), '$id_profile')"); 
                    header('location:sponsor-page.php?key='.$key.'#make-post');
                }
                }
                    ...
                    echo '<div id="focus_post_delete">';
                        echo '<div id="del_post" class="shadow">';
                            ...
                            echo '<div id ="del_post_options">';
                                echo '<a href="sponsor-page.php?key='.$key.'&del='.$id.'">Yes</a>';
                                echo '<a href="sponsor-page.php?key='.$key.'#delete_post" onclick="display_less_post()">No</a>';
                            echo '</div>';
                        echo '</div>';
                    echo '</div>';
                    }
                if(isset($_GET['del'])){
                    $id = $_GET['del'];
                    $res = mysqli_query($dbc,"DELETE FROM sponsor_make_post WHERE id='$id'");
                    if($res){
                    header('location:sponsor-page.php?key='.$key.'#delete_post');
                    }
                }

                $post_result = mysqli_query($dbc,"SELECT * FROM sponsor_make_post WHERE user_id = '$id_profile'");
